...
class BidsMriNidm(BidsNidm):
    def __init__(self, directory, json_map=None, github=None, key=None, owl=None):
        ...
        self._acquisition_phenotype()



    def _acquisition_phenotype(self):
        ...
        for subject_id in self.bids_layout.get_subjects():
            ...
            self._adding_mracquisition(subject_id=subject_id)
            self._adding_phenotype(subject_id=subject_id)



    def _adding_mracquisition(self, subject_id):
        for file_tpl in self.bids_layout.get(subject=subject_id, extensions=['.nii', '.nii.gz']):
            ...
            if subject_id not in self.participant:
                ...
                self.participant[subject_id]['person'] = acq.add_person(
                    attributes=({Constants.NIDM_SUBJECTID: subject_id}))

            # add qualified association with person
            acq.add_qualified_association(person=self.participant[subject_id]['person'],
            ...
            if file_tpl.modality == 'anat':
                ...
                if file_tpl.type in BIDS_Constants.scans:
                    ...
                    logging.info(
                        "WARNING: No matching image contrast type found in BIDS_Constants.py for %s" % file_tpl.type)
                ...
                if file_tpl.modality in BIDS_Constants.scans:
                    ...
                    logging.info(
                        "WARNING: No matching image usage type found in BIDS_Constants.py for %s" % file_tpl.modality)
                # add file link
                        ...
                        if key in BIDS_Constants.json_keys:
                            if type(json_data[key]) is list:
                                acq_obj.add_attributes({BIDS_Constants.json_keys[key.replace(" ", "_")]: ''.join(
                                    str(e) for e in json_data[key])})
                            else:
                                acq_obj.add_attributes(
                                    {BIDS_Constants.json_keys[key.replace(" ", "_")]: json_data[key]})
                ...
                if 'run' in file_tpl._fields:
                    events_file = self.bids_layout.get(subject=subject_id, extensions=['.tsv'],
                                                       modality=file_tpl.modality, task=file_tpl.task, run=file_tpl.run)
                else:
                    events_file = self.bids_layout.get(subject=subject_id, extensions=['.tsv'],
                                                       modality=file_tpl.modality, task=file_tpl.task)
                ...
                if events_file:
                    ...
                    events_obj.add_attributes({PROV_TYPE: Constants.NIDM_MRI_BOLD_EVENTS,
                                               BIDS_Constants.json_keys["TaskName"]: json_data["TaskName"],
                                               Constants.NIDM_FILENAME: self._getRelPathToBIDS(
                ...
                acq_obj_bval.add_attributes(
                    {Constants.NIDM_FILENAME: getRelPathToBIDS(self.bids_layout.get_bval(file_tpl.filename))})
                ...
                acq_obj_bvec.add_attributes(
                    {Constants.NIDM_FILENAME: getRelPathToBIDS(self.bids_layout.get_bvec(file_tpl.filename))})
            else:
                raise Exception(
                    "Wrong modality of the file: {}, it has to be anat, func or dwi".format(file_tpl.modality))

                # link bval and bvec acquisition object entities together or is their association with DWI scan...


    def _adding_phenotype(self, subject_id):
            ...
            with open(tsv_file) as phenofile:
                ...
                for row in pheno_data:
                    ...
                    if subjid[1] != subject_id:
                        ...
                        data_dict = os.path.join(self.directory, "phenotype",
